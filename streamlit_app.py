...
caminho_css = Path(__file__).parent / "static" / "style.css"
...
def carregar_javascript():
    caminho_js = Path(__file__).parent / "static" / "script.js"

    if not caminho_js.exists():
        return

    codigo_js = caminho_js.read_text(encoding="utf-8")

    st.html(
        f"<script>{codigo_js}</script>",
